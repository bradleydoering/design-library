...
export default function SwapItemCard({
  ...
  return (
    <>
      <div
        className={`bg-white  p-4 pt-[0px] pb-[12px] relative shadow-sm border-2 rounded-[20px] border-[#F6F7F9] flex flex-col h-full hover:shadow-md transition-shadow ${
      ...
      >
        ...
        {item.priceIndicator && (
          <div className="absolute top-4 left-3">
            <span className="bg-gray-100 px-2 py-[8px] rounded-xl text-gray-700 font-semibold text-sm">
        ...
        <div className="absolute top-[46px] right-4 flex flex-col gap-1">
          {availableColors.slice(0, maxDisplayColors).map((cObj, idx) => (
            <div
              key={idx}
              className="w-4 h-4 rounded-full border border-gray-200 relative"
        ...
        <div className="mt-auto pt-3">
          {isSelected ? (
            <div className="flex w-[32px] h-[32px] ml-auto items-center justify-center bg-[#8DBACE] rounded-full">
              <Check className="w-5 h-5 text-white" />
            </div>
          ) : (
            <button
              onClick={onPreview}
              className="w-full flex items-center justify-center gap-2 px-4 py-1.5 bg-[#EFEADF] rounded-[10px] text-[#2d332c] hover:bg-[#E5E0D5] transition-colors"
            >
