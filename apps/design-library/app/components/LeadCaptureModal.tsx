...
const LeadCaptureModal = ({ isOpen, onClose, onComplete }: LeadCaptureModalProps) => {
  ...
  const saveLeadData = async (stepData: Partial<LeadFormData>, isPartialSave = true) => {
    try {
      if (leadId) {
  ...
  const nextStep = async () => {
    if (await validateStep(currentStep)) {
      // Always save data after successful validation (handles updates from back navigation)
      if (currentStep === 1) {
        // Save/update step 1 data (contact details) - will create new record or update existing
        await saveLeadData({
        ...
        // Save/update step 2 data (phone and city) - will update existing record
        await saveLeadData({
  ...
  const handleSubmit = async () => {
    if (await validateStep(currentStep)) {
      ...
      // Save final step data (project description) as complete submission
      await saveLeadData({
        projectDescription: formData.projectDescription
      }, false); // isPartialSave = false for complete submission
